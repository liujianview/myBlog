...
$('.superAdminList .superAdminClick').click(function () {
    ...
    $('#statistics,#articleManagement,#articleThumbsUp,#articleCategories,#friendLink,#pictureManagement,#rewardManagement,#userFeedback,#privateWord').css("display","none");
...
//点击照片管理
$('.superAdminList .pictureManagement').click(function () {
    $.ajax({
        type:'post',
        url:'/getPictureInfo',
        dataType:'json',
        ...
        success:function (data) {
            if (data['status'] == 103){
                dangerNotice(data['message'] + " 获取照片失败");
                return;
            }
            var pictureContent = $('.pictureContent');
            pictureContent.empty();
            pictureContent.append($('<div class="contentTop">照片总数：' +
                '<span class="pictureNum"> </span>张' +
                '<div class="updateReward">' +
                '<a class="addPicture"><i class="am-icon-plus-square"></i> 添加照片</a>' +
                '</div>' +
                '</div>'));
            var table = $('<table class="table am-table am-table-bd box-shadow am-table-hover"></table>');
            var thead = $('<thead>' +
                '<tr>' +
                '<th>照片名称</th><th style="width: 10px;">照片路径</th><th>发布日期</th><th class="am-hide-sm-down">照片描述</th><th>操作</th>' +
                '</tr>' +
                '</thead>');
            var pictureTable = $('<tbody id="pictureTable"></tbody>');
            var number = 0;
            if (data['data'].length > 0 && data['status'] == 0){
                $.each(data['data'], function (index,obj) {
                    var tr = $('<tr id=p' + obj['id'] +  '></tr>');
                    tr.append($('<th class="pictureName">'+ obj['pictureName'] +'</th>'));
                    number += 1;
                    tr.append($('<th class="pictureUrl" style="width: 10px;">' + obj['pictureUrl'] + '</th>' +
                        '<th class="pictureDate">'+ obj['pictureDate'] + '</th>' +
                        '<th class="pictureDesc am-hide-sm-down">'+ obj['pictureDesc'] + '</th>' +
                        '<th>' +
                        '<div class="am-dropdown" data-am-dropdown="">' +
                        '<button class="pictureManagementBtn articleEditor am-btn am-btn-secondary">编辑</button>' +
                        '<button type="button" class="deletePicture articleDelete am-btn am-btn-danger">删除</button>' +
                        '</div>' +
                        '</th>'));
                    pictureTable.append(tr);
                })
            }
            table.append(thead);
            table.append(pictureTable);
            pictureContent.append(table);
            $('.pictureNum').html(number);

            updatePictureEditBtn();
            updatePictureDelBtn();
            // $('.deleteReward').click(function () {
            //     removeRewardMoneyId = $(this).parent().parent().attr("id").substring(1);
            //     thisRewardMoney = $(this).parent().prev().prev().prev().find('.am-icon-cny').html();
...
// 更新照片删除按钮
function updatePictureDelBtn() {
    $('.deletePicture').click(function () {
        var $this = $(this).parent().parent().parent()
        removePictureId = $this.attr('id').substring(1);
        $('#deletePicture').modal('open');
    })
}

// 更新照片的编辑按钮
function updatePictureEditBtn() {
    $('.pictureManagementBtn').click(function () {
        $('#addPicture').modal('open');
        var $this = $(this).parent().parent().parent();
        var pictureName = $this.find('.pictureName').html();
        var pictureDate = $this.find('.pictureDate').html();
        var pictureDesc = $this.find('.pictureDesc').html();
        var pictureUrl = $this.find('.pictureUrl').html();
        pictureId = $this.attr('id').substring(1);
        $('#pictureName').val(pictureName);
        $('#pictureDate').val(pictureDate);
        $('#pictureDesc').val(pictureDesc);
...
//删除照片
var removePictureId = "";
$('.surePictureDeleteBtn').click(function () {
    $.ajax({
        type:'get',
        url:'/deletePicture',
        dataType:'json',
        ...
        success:function (data) {
            var pictureNum = $('.pictureNum').html();
            if(data['status'] == 101){
                $.get("/toLogin",function(data,status,xhr){
                    window.location.replace("/login");
                });
            } else if(data['status'] == 103){
                dangerNotice(data['message'] + " 删除照片失败")
            } else if (data['status'] == 704){
                successNotice(data['message']);
                $('#p'+removePictureId).remove();
                $('.pictureNum').html(pictureNum-1);
            } else {
                dangerNotice(data['message'])
            }
        },
...
//添加或编辑照片
$('.sureAddPictureBtn').click(function () {
    var formData = new FormData();
    ...
    if($('#pictureUrl')[0].files[0] == undefined){
        dangerNotice("未选择照片");
        return;
    }
    ...
    formData.append('pictureDesc',  pictureDesc);
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            if (data['status'] == 101){
                ...
                dangerNotice(data['message'] + " 添加照片失败")
            } else if(data['status'] == 705){
                successNotice(data['message']);
                $('#p' + pictureId).find($('.pictureName')).html(pictureName);
                $('#p' + pictureId).find($('.pictureDate')).html(pictureDate);
                $('#p' + pictureId).find($('.pictureDesc')).html(pictureDesc);
                $('#p' + pictureId).find($('.pictureUrl')).html(data['data']['pictureUrl']);
            } else if(data['status'] == 703){
                successNotice(data['message'])

                var pictureContent = $('.pictureContent .table tbody');
                var tr = $('<tr id=p' + data['data']['id'] + '></tr>');
                tr.append($('<th class="pictureName">'+ pictureName +'</th>'));
                tr.append($('<th class="pictureUrl">' + data['data']['pictureUrl'] + '</th>' +
                    '<th class="pictureDate">'+ pictureDate + '</th>' +
                    '<th class="pictureDesc am-hide-sm-down">'+ pictureDesc + '</th>' +
                    '<th>' +
                    '<div class="am-dropdown" data-am-dropdown="">' +
                    '<button class="pictureManagementBtn articleEditor am-btn am-btn-secondary">编辑</button>' +
                    '<button type="button" class="deletePicture articleDelete am-btn am-btn-danger">删除</button>' +
